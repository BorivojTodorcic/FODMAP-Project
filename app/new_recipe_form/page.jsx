...
export default function NewRecipeForm() {
	...
	useEffect(() => {
		fetch(DOMAIN_URL + "/api/all-ingredients")
	...
	const handlePreviewRecipeDetails = (e) => {
		...
		function validateRecipeData(data) {
			const requiredFields = [
				...
				"servings",
				"steps",
			]; // Object keys to be validated (skipping 'diet')
	...
	return (
		<Box sx={{ width: "100%" }}>
			...
			>
				...
				<FormControl>
					<FormLabel required id="meal-type">
						Meal Type
					</FormLabel>
					<RadioGroup
						row
				...
				<Autocomplete
					...
					getOptionLabel={(option) =>
						`${option.ingredient_name} (${option.ingredient_detail})`
					}
					...
					>
						...
						<Grid size={4}>
							<TextField
								...
								label="Quantity"
								onChange={(event) =>
									handleIngredientAmount(ingredient.ingredient_id, event)
								}
							/>
						</Grid>
						<Grid size={4}>
							<FormControl>
								...
								<Select
									...
									onChange={(event) =>
										handleIngredientUnit(ingredient.ingredient_id, event)
									}
								>
			...
			<Fragment>
				...
				>
					...
					<DialogContent>
						<DialogContentText id="alert-dialog-description">
							...
							{recipeDetails.diet.map((type) => {
								return (
									<Typography key={type} variant="body1" gutterBottom>
										{type}
									</Typography>
								);
							})}
							...
							{recipeDetails.ingredients.map((ingredient) => {
								return (
									<Typography
										...
										key={ingredient.ingredient_id}
										gutterBottom
									>{`${ingredient.ingredient_id}: ${ingredient.ingredient_name} (${ingredient.ingredient_detail}) ${ingredient.amount} ${ingredient.unit}`}</Typography>
								);
							})}
							...
							{Object.keys(recipeDetails.steps).map((key) => {
								return (
									<Typography
										...
										key={key}
										gutterBottom
									>{`Step ${key}) ${recipeDetails.steps[key]}`}</Typography>
								);
							})}
						</DialogContentText>
					</DialogContent>
					...
					<DialogActions>
						...
						<Button onClick={handleSaveRecipe} autoFocus>
							Save recipe
						</Button>
					</DialogActions>
				</Dialog>
